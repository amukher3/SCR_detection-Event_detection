...
% mu_1(h)= x(1)*(exp((x(2)-h*T)/x(3)) - exp((x(2)-h*T)/x(4)));
% mu_2(h) = x(1)*(exp((x(2)-(h+1)*T)/x(3)) - exp((x(2)-(h+1)*T)/x(4)));
% mu(h)=mu_1(h) - mu_2(h);
...
mu(i)= -x(1)*(exp((x(2)-(i)*T)/x(3))- exp((x(2)-(i)*T)/x(4))) + x(1)*(exp((x(2)-(i+1)*T)/x(3)) - exp((x(2)-(i+1)*T)/x(4))); %no pairwise difference of the samples
Lhood = Lhood + (-1/2*log(2*2*pi*(noise_var))-(((sample_differences(i)-mu(i))^2)/(2*(2*noise_var))));

%% derivative part
dmu_dalpha= -1*(exp((x(2)-i*T)/x(3))-exp((x(2)-i*T)/x(4))) + 1*(exp((x(2)-(i+1)*T)/x(3)) - exp((x(2)-(i+1)*T)/x(4)));           %dmu/d(alpha)
dmu_dtau = -x(1)*((exp((x(2)-i*T)/x(3)))/x(3)-(exp((x(2)-i*T)/x(4)))/x(4)) + x(1)*((exp((x(2)-(i+1)*T)/x(3)))/x(3)-(exp((x(2)-(i+1)*T)/x(4)))/x(4));    %dmu/d(tau)
dmu_dtauOne = x(1)*(exp((x(2)-i*T)/x(3))*((x(2)-i*T)/(x(3)^2)))- x(1)*(exp((x(2)-(i+1)*T)/x(3))*((x(2)-(i+1)*T)/(x(3)^2)));    %dmu/d(tauOne)
dmu_dtauTwo= -x(1)*(exp((x(2)-i*T)/x(4))*((x(2)-i*T)/(x(4)^2))) + x(1)*(exp((x(2)-(i+1)*T)/x(4))*((x(2)-(i+1)*T)/(x(4)^2))) ;      %dmu/d(tauTwo)
