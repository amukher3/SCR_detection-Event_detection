...
% function [Likelihood_under_Null,AUC,scores,Likelihood_under_Alternative,Window,OnsetTimeInstants,tau_mle,SamplingFreqn,Delta,T,OnsetSampleInstants,WindowTimeLength,labels,noise_var,TimeInstantOfFirstSample,loc,mle_estimates] = DevianceCalcTwoPara(user,DataIndex);
...
% % for j=1:length(ys)-1
% % sample_differences(j,runs)= ys(j+1) - ys(j); % taking differecnes of the samples 
% % end  
...
% A=[];
% b=[];
% lb= [.1,0.01]; % lower bound of the estimates:[alpha tau TauOne TauTwo]
% ub= [3,T-.001]; %upper bound of the estimates
% Aeq = []; %for equality constraint
% beq = []; %for equality constraint
% x=zeros(iter,NumOfParameters);
% x0(1,:) = [lb(1),.01]; % initial estiamtes of the parameters
% x0(2,:) = [lb(1),1];
% x0(3,:) = [lb(1),2];
% x0(4,:) = [lb(1),3];
% x0(5,:) = [lb(1),4];
...
% TauOne=[2];
% TauTwo=[.75];
% for j=1:length(TauOne)
% for i=1:iter
% options = optimoptions('fmincon','SpecifyObjectiveGradient',true); 
% [x(i,:),val(runs,i),flag(runs,i)] = fmincon(@(t_prime)Likelihood_Ha_TwoPara(t_prime,sample_differences(:,runs),noise_var,T,TauOne(j),TauTwo(j)),x0(i,:),A,b,Aeq,beq,lb,ub,[],options);
%  %options = optimoptions('fminunc','SpecifyObjectiveGradient',true); 
% %[x(i,:),val(runs,i),flag(runs,i)] = fminunc(@(t_prime)LikelihoodUnderAlternativeWithDerivative(t_prime,sample_differences(:,runs),noise_var,T),x0(i,:),options);
% end
...
% % end
% % options = optimoptions('fmincon','SpecifyObjectiveGradient',true); 
% % [x(i,:),val(runs,i),flag(i)] = fmincon(@(t_prime)LikelihoodUnderAlternative(t_prime,sample_differences(:,runs),noise_var,T),x0(i,:),A,b,Aeq,beq,lb,ub,[],options);
% % end
% %% estimates after optimisation.
% [I_prime(runs,j),I(runs,j)] = min(val(runs,:)); % checking for the minm. value of the Likelihood function
% ML_estimates(j,runs,:) = x(I(runs,j),:); % getting the estimate values(alpha,tau,tauOne,tauTwo) for the corresponding minm. value of the likelihood
% end
% [val2(runs),Ind(runs)] = min(I_prime(runs,:));
% mle_estimates(runs,:)= ML_estimates(Ind(runs),runs,:);
...
%   Likelihood_under_Alternative(runs) = -val2(runs);
...
% 
% end
% %scores=downsample(scores(1:length(scores)),WindowTimeLength/T);
% %labels=downsample(labels(1:length(labels)),WindowTimeLength/T);
% %[pks,loc] = findpeaks(scores); % Threshold is lowered to see if the model actually picks up all the events.
% [X,~,Threshold,AUC] = perfcurve(labels,scores,1);
...
%perfcurve.
%function [Likelihood_under_Null,AUC,X,Y,scores,Threshold,Likelihood_under_Alternative,mle_estimates,Window,NoiseWindows,EventWindows,condn,OnsetTimeInstants,tau_mle,SamplingFreqn,Delta,T,OnsetSampleInstants,WindowTimeLength,labels,noise_var,TimeInstantOfFirstSample,loc] = DevPeakLocations(user,DataIndex)
function [mu_check,Likelihood_under_Null,AUC,scores,Likelihood_under_Alternative,Window,OnsetTimeInstants,tau_mle,SamplingFreqn,Delta,T,OnsetSampleInstants,WindowTimeLength,labels,noise_var,TimeInstantOfFirstSample,loc,mle_estimates] = DevianceCalcTwoPara(user,DataIndex);
[Window,NoiseWindows,EventWindows,condn,OnsetTimeInstants,tau,SamplingFreqn,Delta,T,OnsetSampleInstants,WindowTimeLength,labels,TimeInstantOfFirstSample]= MakingTimeWindows(user,DataIndex);
TotalSizeOfWindow=size(Window,2); % Total number of windows 
NumOfParameters=2;
iter=15; % no. of independent initialisations
...
flag=zeros(TotalSizeOfWindow,iter);
NumOfSamplesInWindow = WindowTimeLength/T;
%sample_differences=zeros(NumOfSamplesInWindow-1,TotalSizeOfWindow); % when doing pairwise
%difference for the samples in the window. 
sample_differences=zeros(NumOfSamplesInWindow,TotalSizeOfWindow);
scores=zeros(1,TotalSizeOfWindow);
mle_estimates=zeros(TotalSizeOfWindow,NumOfParameters);
alpha_mle=zeros(1,TotalSizeOfWindow);
tau_mle=zeros(1,TotalSizeOfWindow);
...
for i=1:iter
%options = optimoptions('fmincon','SpecifyObjectiveGradient',true); 
[x(i,:),val(runs,i),flag(runs,i)] = fmincon(@(t_prime)Likelihood_Ha_TwoPara(t_prime,sample_differences(:,runs),noise_var,T,TauOne,TauTwo),x0(i,:),A,b,Aeq,beq,lb,ub);
 %options = optimoptions('fminunc','SpecifyObjectiveGradient',true); 
%[x(i,:),val(runs,i),flag(runs,i)] = fminunc(@(t_prime)LikelihoodUnderAlternativeWithDerivative(t_prime,sample_differences(:,runs),noise_var,T),x0(i,:),options);
end
%% Doing sample differences in the time windows.
...
%% Calculating the Likelihood values under the Null,Alternative hypothesis,deviance when the samples in the windows are not differenced pairwise. 
  ...
  Likelihood_under_Null(runs)= -(length(sample_differences(:,runs))/2)*log(2*pi*(noise_var)) - (sum((sample_differences(:,runs)).^2))/(2*(noise_var));
 for h=1:length(sample_differences(:,runs))
  mu_Num(h)= alpha_mle(runs)*(exp((tau_mle(runs)- h*T)/TauOne) - exp((tau_mle(runs)- h*T)/TauTwo));
  mu_mle(h)= mu_Num(h);
  Likelihood_values_of_samples(runs,h) = -1/2*log(2*pi*noise_var)-((sample_differences(h,runs)- mu_mle(h))^2)/(2*noise_var);
end
 Likelihood_under_Alternative(runs) = (sum(Likelihood_values_of_samples(runs,:)));
 mu_check(runs)= sum(mu_mle(:));
%  Likelihood_under_Alternative(runs) = -val(runs,I);
  %scores(runs) = 1*(Likelihood_under_Alternative(runs)); % scores are the negative of the Likelihood values under the alternative hypothesis.
 scores(runs) = (Likelihood_under_Alternative(runs) - Likelihood_under_Null(runs)); % Scores are the differences between the Likelihood values

end
%scores=downsample(scores(2:length(scores)),WindowTimeLength/T);
%labels=downsample(labels(2:length(labels)),WindowTimeLength/T);
%[pks,loc] = findpeaks(scores); % Threshold is lowered to see if the model actually picks up all the events.
